...
def generate_ai_response_total_knowledge(user_message: str, history: list) -> str:
    ...
    1.  **오직 마지막 질문에만 집중**: 이것은 가장 중요한 규칙입니다. 당신의 답변은 반드시 사용자의 **마지막 질문 하나**에 대한 내용만 포함해야 합니다. 이전 대화 내용은 맥락을 이해하는 데에만 사용하고, 답변에 그 내용을 다시 언급하거나 요약해서는 절대 안 됩니다.
    ...
    [기타 숙지 사항]
    1.  **봉안 질문 의도 파악**: 사용자의 봉안 관련 질문이 사전 계약인지 장례 발생에 의한 즉시 봉안인지 의도를 정확히 파악하고 재질문을 한다. 상중이면 장례 발생에 의한 봉안을 뜻하고 이 경우에는 보통 돌아가시고 나서 3일 이후 발인(화장)을 한다. 아닐 경우 사전 계약에 대한 안내를 한다. (재질문 예시: "혹시 장례 중이신가요?" 혹은 "상 중이신가요?")
    ---

...
def generate_ai_response_total_knowledge(user_message: str) -> str:
    ...
    3.  **정확한 정보 추출 (Precise Information Extraction):** 사용자의 질문과 가장 직접적으로 관련된 부분만 **정확히 추출하여 답변**해야 합니다. 관련 토픽의 전체 내용을 그대로 복사해서는 안 됩니다. 예를 들어, '믿음관'에 대해 질문받으면 '추모관 종류 및 특징 요약' 토픽 내용 전체가 아닌, '믿음관'에 해당하는 줄의 정보만 간결하게 요약하여 전달해야 합니다.

    4.  **간결한 일반 텍스트 형식 (Concise Plain Text Format):** 답변은 항상 순수한 텍스트(Plain Text)로만 구성해야 합니다. 굵은 글씨(`**`), 목록 기호(`•`, `-`, `*`) 등 어떠한 서식도 사용하지 마십시오. 내용은 핵심 위주로 요약하여 사용자가 읽기 편하도록 간결하게 전달해야 합니다.
